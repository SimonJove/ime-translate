...
M.adapters.libretranslate = {
  ...
  -- Feature 005: only this adapter is guarded. The LLM adapters' prompt already
  -- asks for URLs to be kept (§7.4).
  prepare = function(text) return url_guard.guard(text) end,
  finish = function(out, urls)
    if not urls then return out end
    return url_guard.unguard(out, urls)
  end,
}
...
local function translate(settings, text, runner, api_key)
  ...
  local sent, guard = text, nil
  if ad.prepare then sent, guard = ad.prepare(text) end
  ...
  parts[#parts + 1] = "-d"
  parts[#parts + 1] = json.shq(ad.body(settings, sent))
  ...
  out = trim(out)
  if ad.finish then
    out = ad.finish(out, guard)
    if not out then return false, "bad_guard" end
  end
  return true, out
end
