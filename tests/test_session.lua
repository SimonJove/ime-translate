...
local SH, OPT = shift_tap.SHIFT, shift_tap.RIGHT_OPTION
...
local s = fake_ctx("今天")
eq(session.tap_down(s, SH), nil, "no Shift pending at first")
session.set_tap_down(s, SH, { code = 0xFFE1, at = 123456 })
local d = session.tap_down(s, SH)
...
session.set_tap_down(s, SH, nil)
eq(session.tap_down(s, SH), nil, "and it can be dropped")
eq(s.props["ime_translate.shift_down"], "", "an empty string when none")
eq(session.tap_down(nilctx, SH), nil, "a store that returns nil reads as none")
...
local o = fake_ctx("")
eq(session.tap_down(o, OPT), nil, "no Option down at first")
session.set_tap_down(o, OPT, { code = 0xFFEA, at = 1234 })
local od = session.tap_down(o, OPT)
eq(od and od.code, 0xFFEA, "the Option keycode round-trips")
eq(od and od.at, 1234, "the Option time round-trips")
eq(session.tap_down(o, SH), nil, "the Option state is not the Shift state")
session.set_tap_down(o, SH, { code = 0xFFE1, at = 5 })
eq(session.tap_down(o, OPT).code, 0xFFEA, "and the Shift state does not overwrite it")
session.set_tap_down(o, OPT, nil)
eq(session.tap_down(o, OPT), nil, "the Option state can be dropped")
eq(session.tap_down(o, SH).code, 0xFFE1, "leaving the Shift state")
