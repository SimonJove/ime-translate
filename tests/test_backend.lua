...
sent, urls = backend.guard("没有链接")
eq(sent, "没有链接", "no URL: the text as is"); eq(urls, nil, "no URL: no list")
sent, urls = backend.guard("X_1 和 https://a.com")
...
sent, urls = backend.guard("设置 MAX_1 见 https://a.com")
eq(sent, "设置 MAX_1 见 https://a.com", "a draft holding MAX_1 is sent unguarded"); eq(urls, nil, "with no list")

eq(backend.unguard("Take a look at X_1 this repo", { G1 }), "Take a look at " .. G1 .. " this repo", "unguard restores")
eq(backend.unguard("A X_2 and X_1.", { "https://a.com", "https://b.com/%1" }),
   "A https://b.com/%1 and https://a.com.", "order in the output is free; % in a URL is literal")
eq(backend.unguard("nothing here", { G1 }), nil, "a missing placeholder fails")
eq(backend.unguard("X_1 X_1", { G1 }), nil, "a doubled placeholder fails")
eq(backend.unguard("X_12", { G1 }), nil, "X_12 is not X_1")
eq(backend.unguard("in X_1", { G1 }), "in " .. G1, "a spaced placeholder keeps its one space")
eq(backend.unguard("inX_1.", { G1 }), "in " .. G1 .. ".", "a placeholder glued to a word gets its space back")
eq(backend.unguard("X_1a", { G1 }), nil, "X_1 followed by a letter is not X_1")
-- 005 Task 2 review, yellow: a URL right after a letter is spaced off, so its
-- placeholder is not read as part of the word
sent, urls = backend.guard("seehttps://a.com 看看")
eq(sent, "see X_1 看看", "a URL after a letter: the placeholder is spaced off")
eq(backend.unguard("See X_1 look", urls), "See https://a.com look", "and comes back")
sent = backend.guard("v2https://a.com")
eq(sent, "v2 X_1", "after a digit too")
sent = backend.guard("看https://a.com")
