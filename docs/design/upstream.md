...
| F29 | librime `switches.cc` `Switches::GetStateLabel`, `switches.h` `StringSlice::operator bool`; `gear/switch_translator.cc` `has_state_label`, `SwitchTranslation::LoadSwitches`; `context.cc` `Context::set_option`; `engine.cc` `ConcreteEngine::OnOptionUpdate`; `rime_api_impl.h` `RimeGetStateLabelAbbreviated` (tag 1.16.0). Squirrel `SquirrelApplicationDelegate.swift` `notificationHandler`, `showStatusMessage`; `SquirrelPanel.swift` `update`, `updateStatus`; `SquirrelInputController.swift` `showPanel` call; `MacOSKeyCodes.swift` `osxKeycodeToRime`, `handle(_:client:)` `.keyDown` (tag 1.1.2) | `set_option` stores the value and always fires the option notifier, changed or not. The engine then refreshes the non-confirmed composition if one is open, and sends `option` with the name, prefixed `!` when off. Squirrel asks for the state label of that option from the session's schema `switches`, and shows it only if it is non-empty. A label is a `StringSlice`, true only with a non-null pointer and a non-zero length, so an empty string in `states` is no label. The switcher menu lists a switch only if its state 0 has a label. So a switch declared as `states: ["", X]` is left out of the menu, turning it off shows nothing, and turning it on shows X. The panel shows a status message only when it has no candidates and no preedit to draw, and otherwise discards it. With inline preedit the controller passes it an empty preedit. A letter typed with Control and Shift reaches Rime as its uppercase keysym when `charactersIgnoringModifiers` gives the uppercase letter, or gives a lowercase one with Shift and not Caps Lock. A lowercase one with Shift and Caps Lock both on arrives lowercase. Read by the feature 003 design, 2026-09-22. **Correction, 003 Task 5's review:** Squirrel's `notificationHandler` reads the label with `String(cString:)`, which ignores the slice's length, so on 1.1.2 the short label is the whole state text even without `abbrev`. The schema keeps `abbrev`: it is right for any frontend that honours the length, and harmless here | §5.6: the notice switches, the hotkey's two keysyms |
| F30 | Squirrel `MacOSKeyCodes.swift` `keycodeMappings`; `SquirrelInputController.swift` `handle(_:client:)`, the `.flagsChanged` and `.keyDown` cases (tag 1.1.2) | Right Option (`kVK_RightOption`) reaches Rime as `XK_Alt_R` (0xFFEA), and Left Option as `XK_Alt_L`. Right Command is `XK_Super_R`, and Right Control is `XK_Control_R`. As with Shift (F21), the press carries the mask after the change, so Right Option's press has the Alt bit (0x8); the release has the mask without it, plus `kReleaseMask`. The `.keyDown` case hands every key without Command to Rime, with or without a composition, so a key lost with no draft open was lost before Squirrel. Read by the feature 004 design, 2026-09-22 | §5.6: the Right Option tap |
